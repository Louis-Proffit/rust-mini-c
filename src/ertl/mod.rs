...
use crate::ertl::structure::register::{CALLEE_SAVED, PARAMETERS, Register, RESULT};
...
fn ertl_fun<'a>(fun: &rtl::Fun<'a>) -> ErtlResult<Fun<'a>> {
    ...
    let callee_saved_regs = CALLEE_SAVED
        .iter()
        .map(|x| { (x.clone(), Register::Virtual(rtl::register::Register::fresh())) })
        .collect::<Vec<(Register, Register)>>();
    ...
    for (callee_saved, pseudo) in &callee_saved_regs {
        callee_restore_lbl = body.insert(Instr::EMBinop(Mbinop::MMov, pseudo.clone(), callee_saved.clone(), callee_restore_lbl.clone()));
    }
    body.insert_at_label(&fun.exit, Instr::EMBinop(Mbinop::MMov, fun.result.clone().into(), Register::Rax, callee_restore_lbl));
    ...
    for (callee_saved, pseudo) in &callee_saved_regs {
        callee_save_lbl = body.insert(Instr::EMBinop(Mbinop::MMov, callee_saved.clone(), pseudo.clone(), callee_save_lbl.clone()));
    }
...
fn ertl_instr<'a>(graph: &mut Graph<'a>, label: &Label, instr: &rtl::Instr<'a>) -> ErtlResult<()> {
    match instr {
        ...
        rtl::Instr::EMBinop(Mbinop::MDiv, r1, r2, l) => {
            let post_mov_label = graph.insert(Instr::EMBinop(Mbinop::MMov, Register::Rax, r1.clone().into(), l.clone()));
            let div_lbl = graph.insert(Instr::EMBinop(Mbinop::MDiv, r1.clone().into(), Register::Rax, post_mov_label));
            graph.insert_at_label(label, Instr::EMBinop(Mbinop::MMov, r2.clone().into(), Register::Rax, div_lbl))
        }
        ...
        rtl::Instr::ECall(r, name, args, l) => {
            ...
            let rsp_operation_lbl = graph.insert(Instr::EMUnop(Munop::Maddi((args_on_stack * 8) as Value), Register::Rsp, l.clone()));
            let result_lbl = graph.insert(Instr::EMBinop(Mbinop::MMov, RESULT, r.clone().into(), rsp_operation_lbl));

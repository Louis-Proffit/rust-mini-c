...
use rust_mini_c::interpreter::rtl::interp_rtl;
use rust_mini_c::parser::parse_file;
use rust_mini_c::rtl::rtl_file;
...
fn _test_exec_bad(path: &str) {
    ...
    let rtl = rtl_file(&file).unwrap();
    let interp = interp_rtl(&rtl);
...
fn _test_exec_good(path: &str, expected_path: &str) {
    ...
    let rtl = rtl_file(&file).unwrap();
    let interp = interp_rtl(&rtl).unwrap();
